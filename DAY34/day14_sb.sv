  ...
  task run();
    ...
    forever begin
      ...
      if (mon_item.gnt !== gnt(drv_item.req)) begin
        $fatal(1, "%t Output doesn't match! Expected: %b Got: %b", $time, mon_item.gnt, gnt(drv_item.req));
      end
      else begin
        $display("%t [SCOREBOARD] Output matches the expected output. Expected: %b Got: %b", $time, mon_item.gnt, gnt(drv_item.req));
      end
    end
